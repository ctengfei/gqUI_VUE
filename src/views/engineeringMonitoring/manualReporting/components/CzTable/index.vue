...
import {
  deleteDamManualReportWyR,
  getDamBaseList,
  getDamManualReportWyR, getDamStBase,
  postDamManualReportWyR,
...
const getData = () => {
  getDamManualReportWyR({...serachForm.value,pageNum: currentPage.value, pageSize: pageSize.value}).then(res => {
    if (res.success) {
...
const submit = defineCrudSubmit((close, done, type, isValid, invalidFields) => {
  if (isValid) {
    if (type == 'add') {
      ...
      putDamManualReportWyR(form.value.id, {
        ...form.value,
        resCode:form.value.node[0],
        damCode:form.value.node[1],
        stType:form.value.node[2],
...
const deleteRow = (row) => {
  ElMessageBox.confirm(`是否确认删除数据项？`, "系统提示", {
    ...
    deleteDamManualReportWyR(row.id).then(res => {
      if (res.success) {
        getData()
        ElMessage({
          message: '删除成功',
...
const exportData = () => {
  proxy.download("/business/standard/damManualReportWyR/page/export", {
    ...serachForm.value,pageNum:currentPage.value,pageSize:pageSize.value
  },`垂直位移数据_${dayjs().format('YYYY-MM-DD')}.xlsx`);
}
</script>
