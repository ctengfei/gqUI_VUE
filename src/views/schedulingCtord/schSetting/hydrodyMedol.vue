<template>
  <div class="" v-if="visible">
    <div class="head-title" style="border:0;margin-bottom: 10px;border-bottom: 1px solid #e6e6e6;"><span>水动力模型方案</span></div>
    <pro-crud
      ref="crudRef"
      v-model="form"
...
import { addPlan } from "@/api/schedulingCtord/predOptions";
import hydrodyModelSet from "./components/hydrodyModelSet";
...
onMounted(() => {
  ...
  // getShList().then((res) => {
  //   if (res.success) {
  //     radios1.value = res.data
  //       .reverse()
  //       .map((r) => ({ label: r.wshName, value: r.wshCode }));
...
//   }else{
//     getBnchHydroSluiceList().then(res=>{
