<template>
  ...
  <div class="card-container" v-show="selectLocal == 1 || selectLocal == 0">
    <li class="tab_1">
    ...
    <!-- <li class="tab_4">
      ...
      <div class="item-box" style="font-size: 13px">
        <div class="item">
          <span style="margin-bottom: 5px">洪量(m³)</span>
          <span>{{ statistics.floodVolume || 0 }}</span>
        </div>
        <div class="item">
          <span style="margin-bottom: 5px">峰现时间</span>
          <span>{{ statistics.maxQTm || '-' }}</span>
        </div>
        <div class="item">
          <span style="margin-bottom: 5px">最大流量 (m³/s)</span>
          <span style="font-size: 16px">{{ statistics.maxQ || 0 }}</span>
        </div>
      </div>
    </li> -->
    <li class="tab_4">
      ...
      <li class="tab_5" v-show="selectLocal == 1">
        ...
        <div class="item-box" style="font-size: 13px">
          ...
          <div class="item">
            <span style="margin-bottom: 5px">超警戒个数</span>
            <span>{{ floodForecast.beyondAlarmHours || 0 }}</span>
          </div>
          <div class="item">
            <span style="margin-bottom: 5px">超警戒小时</span>
  ...
  <div class="head" v-show="selectLocal == 1 || selectLocal == 0">
    
    <div v-if='!selectType' style="display:flex;align-items: center;margin-left: 20px;">
      <el-button type="primary" @click="selectLocal = 2">入渠山洪预报结果</el-button>
    </div>
    <div v-else style="display:flex;align-items: center;margin-left: 20px;">
      <el-button type="primary" @click="selectLocal = 3">渠道洪水演进结果</el-button>
    </div>
    <div style="float: right;display:flex;align-items: center;">
      <span style="margin-right: 20px"><strong>方案名称：</strong>{{ forecastPeriod.caseName }}</span>
      <span style="margin-right: 20px"><strong>预报时段</strong>：{{
        `${forecastPeriod.forecastSTime} - ${forecastPeriod.forecastETime}`
        }}</span>
        <!-- 山洪 v-if='!selectType'-->
        <el-button style="margin-right: 20px" v-if='!selectType' type="success" @click="handleCompute" :disabled="disabled">方案计算</el-button>
        <!-- 演进 -->
        <el-button style="margin-right: 20px" v-else type="success" @click="handleComputeEvolution" :disabled="disabled">方案计算</el-button>
    </div>
    <div style="display:flex;align-items: center;">
      <el-button type="primary" link icon="ArrowLeftBold" @click="goBack">返回</el-button>
    </div>
  </div>
  <div v-show="selectLocal == 1 || selectLocal == 0" class="real-boo-container">
    <div class="lf" ref="channelMap" id="channelMap">
    </div>

    <!-- <div class="rg" v-show="!selectType">
      <div class="head-title">山洪入渠断面</div>
      <div ref="hdTable" style="flex:1">
        <el-table :data="tableData4" height="670" border @row-click="handleRowChange">
    ...
    <div class="rg">
      <div v-if="selectType">
        <div class="head-title" style="display: flex; justify-content: space-between">
          <span>渠道纵剖面</span>
          <el-date-picker v-model='channelDateTimes' type="datetime" value-format="YYYY-MM-DD HH:mm:ss"
            format="YYYY-MM-DD HH:mm:ss" placeholder='请选择开始时间' :clearable='true' @change="channelTimeChange" />
        </div>
        <div ref="channelEchart" id="channelEchart" style="width: 100%;height: 200px;"></div>
          <!-- <lineChart :lineData='lineData' /> -->
      </div>
      <div class="head-title">{{ selectType ? '渠道断面' : '山洪入渠断面' }}</div>
      <div ref="hdTable" style="flex:1" v-if="selectType">
        <el-table :data="chnnelTable" height="670" border @row-click="handleRowChange">
          <el-table-column prop="bnch" label="断面名称" align="center"/>
          <el-table-column prop="chanName" label="渠道名称" align="center"/>
          <el-table-column prop="highestWater" label="最高水位(m)" align="center"/>
          <el-table-column prop="alarmWater" label="警戒水位" align="center"/>
          <el-table-column prop="highestTime" label="最高水位时间" align="center"/>
        </el-table>
      </div>
      <div ref="hdTable" style="flex:1" v-else>
        <el-table :data="tableData4" height="670" border @row-click="handleRowChange">
          <el-table-column prop="bnchName" label="断面名称" align="center" />
          <el-table-column prop="maxQ" label="洪峰流量(m³/s)" align="center" />
          <el-table-column prop="floodVolume" label="洪量(m³)" align="center" />
  ...
  <div v-show="selectLocal == 3" class="real-hydrody-container">
    <div style="margin-bottom: 10px; display: flex; justify-content: space-between">
      <div style="display: flex; align-items: center">
        ...
        <span style="margin-left: 20px"><strong>预报时段</strong>：{{
          ...
          <span style="margin-left: 20px"><strong>方案名称：</strong>{{ forecastPeriod.caseName }}</span>
      </div>
      <div>
        <el-button type="primary" link :icon="ArrowLeftBold" @click="selectLocal = 1">返回</el-button>
      </div>
    </div>
    <div class="hydrody">
      <div class='lineChartDv'>
        <div>
          <span>时间：</span>
          <el-date-picker v-model='chanResTimeModel' type="datetime" value-format="YYYY-MM-DD HH:mm:ss"
            format="YYYY-MM-DD HH:mm:ss" placeholder='请选择开始时间' :clearable='true' />
      ...
      <div class='lineChartDv'>
        <div>
          ...
          <el-select v-model="bnchCodeModel">
            <el-option v-for="item in bnchArr" :key="item.bnch" :label="item.bnch" :value="item.bnch" />
          </el-select>
          <el-button type='primary' style='margin-left: 20px' @click='chanResTimeModelChange(1)'>查询</el-button>
        </div>
        <div>
          <signLineChart class="chartsLine" :lineData="bnchEchartData" />
        </div>
      </div>
    </div>
  </div>
  <div v-show="selectLocal == 2" class="real-time-container" ref="realTimeContainer">
    <div
      style="padding-bottom: 10px; display: flex; justify-content: space-between; border-bottom: 1px solid #e6e6e6; box-sizing: border-box;">
      <div style="display: flex; align-items: center">
...
<script setup>
import {nextTick, onMounted, ref, watch, onBeforeUnmount, defineProps, defineEmits} from 'vue';
import {
  getBaseData, getBasinBachList, getCardForecaset,
  getChanChart, getForecastResultListByBnch, getPlanForecastSectionList,
  getRainInfo,
  getRainShChart,
  getRainTable,
  getReachInfo,
  ...
  getPreAlarmList,
  getChaneParmList,
  getFloodSlldModelForecast
} from "@/api/schedulingCtord/realtimeFor";
import useDictionaryStore from "@/store/modules/dictionary";
import {getPlanList, getShList, detailParamHydrostart, getChanFloodReultBy} from "@/api/schedulingCtord/schSetting";
...
import {getSchemeCalculation} from "@/api/schedulingCtord/predOptions";
import lineChart from '../lineChart';
import signLineChart from '../signLineChart';

const props = defineProps(['visible','planCode','selectType', 'select1'])
const emit = defineEmits(['visible', 'update:select1']);
...
let selectLocal = ref(props.select1)
let lineData = ref({});
let modelDom = ref(1);
...
let chnnelTable = ref([]);
let channelDateTimes = ref(dayjs().format('YYYY-MM-DD HH:mm:ss'));
let chanResTimeModel = ref(dayjs().format('YYYY-MM-DD HH:mm:ss'));
...
onMounted(async()=>{
  ...
  map.value && map.value.on('click', (event) => {
    map.value.forEachFeatureAtPixel(event.pixel, (feature, layer) => {
      ...
      if (properties.types == 'param') {
        ...
        dialogTitle.value = properties.rvskName;
        dialogVisible.value = true;
        getResultBnchHydro({
 ...
 /* parmAlarmStList().then(res=>{
    vector.getFeatures().forEach(r => {
        if (r.getProperties().types == 'param') {
          vector.removeFeature(r)
        }
      })
      const features = res.data.map(r => {
  ...
  if(!props.select1) {
    getPlanList({
      caseCode: props.planCode
    }).then(res => {
      if (res.success) {
        forecastPeriod.value = res.data.data.filter(r => r.caseCode == props.planCode)[0]
        // 天气统计
        getCardForecaset({
        ...
        getPlanForecastSectionList(dmparams).then(res=>{
          if(res.success){
    ...
    await detailParamHydrostart(props.planCode).then((res) => {
      if (res.success) {
        forecastPeriod.value = { forecastSTime: res.data.starttime,forecastETime:res.data.endtime, caseName: res.data.caseName }
  ...
  // 山洪预报预警 floodForecastAlarm,
  floodForecastAlarm().then(res=>{
  ...
  channelTimeChange(forecastPeriod.value.forecastSTime)
})
...
// 演进方案
const handleComputeEvolution = () => {
    disabled.value = true;
    downloadLoadingInstance = ElLoading.service({ text: "数据计算中，请稍候", background: "rgba(0, 0, 0, 0.7)", });
    getFloodSlldModelForecast({caseCode: props.planCode,endTime: forecastPeriod.value.forecastETime, startTime: forecastPeriod.value.forecastSTime}).then(res=>{
        if(res.success){
...
const handleCompute = () => {
    disabled.value = true;
    downloadLoadingInstance = ElLoading.service({ text: "数据计算中，请稍候", background: "rgba(0, 0, 0, 0.7)", });
    getSchemeCalculation({caseCode: props.planCode,endTime: forecastPeriod.value.forecastETime, startTime: forecastPeriod.value.forecastSTime}).then(res=>{
        if(res.success){
            const timer = setTimeout(()=>{
                clearTimeout(timer);
                disabled.value = false;
...
const computeData = () => {
  if(props.select1 == 1){ // 杨武干渠
    ...
    //山洪径流图
    getRainShChart({caseCode: props.planCode,}).then(async (res) => {
      if (res.success) {
        let mergedData = {};
        res.data.forEach(r => {
    ...
    // 山洪片区面雨量
    getRainTable({caseCode:props.planCode,}).then(res=>{
      if(res.success){
...
const chanResTimeModelChange = (val) => {
  if(!val){
    ...
    getChanFloodReultBy(
      ...
      let y1Data = [];
      let xLine = "时间"
      xData = temp.map(item=>item.tm);
      yData = temp.map(item=>item.z);
      y1Data = temp.map(item=>item.z);
      bnchEchartData.value = { xData, yData,y1Data, xLine }
    })
  }
}

watch([selectLocal,hdTable],([s1,hd])=>{
  if(s1 == 1 && hd){
...
watch([props.select1,select2],([s1,s2])=>{
