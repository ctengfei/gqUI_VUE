<template>
  <div style="padding: 20px 20px;height: 100%;overflow: auto">
    <div class="year-filter">
      <span>年份：</span>
      <a-range-picker v-model:value="year" picker="year" valueFormat="YYYY" :placeholder="['开始年份','结束年份']" />
    </div>
...
import {getYear} from "@/api/waterManager/yearWaterAnalysis";
const year =ref([dayjs().subtract(4,'year').format('YYYY'),dayjs().format('YYYY')]);
let index = ref(1);
let items = ref({
  agriculture: "0",
  ecology: "0",
...
onMounted(()=>{
  watchEffect(()=>{
    ...
    getYear({startTime:year.value[0], endTime:year.value[1], waterUse:index.value}).then(res=>{
      if(res.success){
    ...
    getYear({startTime:year.value[0], endTime:year.value[1], waterUse:null}).then(res=>{
      if(res.success){
        const data = res.data;
        items.value = {
