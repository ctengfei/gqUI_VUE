...
const submit = defineCrudSubmit((close, done, type, isValid, invalidFields) => {
  if(isValid){
    if(type == 'add'){
      addPlan({...form.value,waterUseType:'3'}).then(res=>{
        if(res.success){
          ElMessage({
            message: '添加成功',
            type: 'success',
      ...
      updatePlan(form.value.id,{...form.value,waterUseType:'3'}).then(res=>{
        if(res.success){
          ElMessage({
            message: '修改成功',
            type: 'success',
