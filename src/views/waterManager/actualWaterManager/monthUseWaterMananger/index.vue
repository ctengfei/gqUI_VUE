...
import {
  ...
  getYearStatistics,
  updateActua
} from "@/api/waterManager/actualWaterManager";
...
const form = ref({});
const crudRef = ref();
const serachForm = ref({month:dayjs().format('YYYY')});
let data = ref([]);
...
const exportData = () => {
  proxy.download("/business/waterUse/resInflowWreqMonth/monthStatisticsExport", {
    year:serachForm.value.month
  },`月实际用水统计_${dayjs().format('YYYY-MM-DD')}.xlsx`);
}
</script>
