      ...
      <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="操作" align="center" width="240" class-name="small-padding fixed-width">
            <template #default="scope">
               ...
               <el-dropdown @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['monitor:job:changeStatus', 'monitor:job:query']" style="margin-top: 5px;margin-left: 5px;">
                  ...
                  <template #dropdown>
                     <el-dropdown-menu teleported="true">
                        <el-dropdown-item command="handleRun" icon="CaretRight"
                           v-hasPermi="['monitor:job:changeStatus']" teleported>执行一次</el-dropdown-item>
                        <el-dropdown-item command="handleView" icon="View"
                           v-hasPermi="['monitor:job:query']" teleported>任务详细</el-dropdown-item>
                        <el-dropdown-item command="handleJobLog" icon="Operation"
                           v-hasPermi="['monitor:job:query']" teleported>调度日志</el-dropdown-item>
                     </el-dropdown-menu>
                  </template>
               </el-dropdown>
...
function handleSelectionChange(selection) {
  ...
  jobNameArr.value = selection.map(item => item.jobName);
...
function handleJobLog(row) {
  ...
  router.push('/system/monitor/job-log/index/' + jobId)
}
/** 新增按钮操作 */
function handleAdd() {
  reset();
...
function handleDelete(row) {
  ...
  const jobNames = row.jobName || jobNameArr.value;
  proxy.$modal.confirm('是否确认删除定时任务编号为"' + jobNames + '"的数据项?').then(function () {
