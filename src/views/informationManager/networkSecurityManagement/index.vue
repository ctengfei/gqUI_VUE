...
import {
  addNetworkSafety,
  delNetworkSafety, getNetworkSafety,
  updateNetworkSafety
} from "@/api/informationManager/networkSecurityManagement";
...
const formColumns = defineFormColumns(computed(() => [
  {
    ...
    prop: 'checkItem',
    component: 'el-input',
    rules: [{required: true, message: '请输入检查事项'}],
    props: {
      placeholder: '请输入检查事项',
    },
    md: 12
...
const submit = defineCrudSubmit((close, done, type, isValid, invalidFields) => {
  if (isValid) {
    ...
    if (type == 'add') {
      ...
      updateNetworkSafety(form.value.id, {
        ...form.value,
        checkUserName:name.label|| '',
        attachment: JSON.stringify(fileList.value),
      }).then(res => {
...
const deleteRow = (row) => {
  ElMessageBox.confirm(`是否确认删除数据项？`, "系统提示", {
    ...
    delNetworkSafety(row.id).then(res => {
      if (res.success) {
        getData()
        ElMessage({
          message: '删除成功',
...
const exportData = () => {
  proxy.download("/business/standard/networkSafety/page/export", {
    ...serachForm.value,
    pageNum: currentPage.value, pageSize: pageSize.value
  }, `网络安全管理_${dayjs().format('YYYY-MM-DD')}.xlsx`);
}
</script>
