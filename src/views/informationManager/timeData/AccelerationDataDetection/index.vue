<template>
  ...
  import {gettblJcJsd} from "@/api/informationManager/timeData/AccelerationDataDetectionTimeData";
    ...
    },
  {
        ...
        props: {
            placeholder: '请输入名称',
            clearable: true,
            data:[{label:'全部',value:''},{label:'GNSS全站仪',value:'GNSS全站仪'},{label:'无线裂缝计',value:'无线裂缝计'}],
    ...
    {
        label: '一个采样周期内X轴方向加速度的最大变化量（mg）',
        prop: 'gx',
        search: false,
        form: true
    },
    {
        label: '一个采样周期内Y轴方向加速度的最大变化量（mg）',
        prop: 'gy',
        search: false,
        form: true
    },
    {
        label: '一个采样周期内Z轴方向加速度的最大变化量（mg）',
        prop: 'gz',
        search: false,
        form: true
    },
  ...
  const exportData = () => {
    proxy.download("/business/huangtangMonitor/tblJcJsd/page/export", {
      ...serachForm.value,
      pageNum:currentPage.value,
      pageSize:pageSize.value,
      startTime:serachForm.value.tm ? serachForm.value.tm[0] : '',
      endTime:serachForm.value.tm ? serachForm.value.tm[1] : '',
      tm:null,
    },`加速度数据监测_${dayjs().format('YYYY-MM-DD')}.xlsx`);
  }
  </script>
