...
// 判断后缀
export function checkSuffix(file,suffix=['png','jpg','jpeg']) {
  let suff = file.name.split('.').slice(-1);
  if(suff.length){
    return suffix.some(r=> r.toLowerCase() === suff[0].toLowerCase());
  }else{
    return false;
  }
}
...
//根据id去重
export function filterID(arr,key) {
  let obj = {};
...
// 格式化tree
export function loopTree(arr) {
  return arr.map(r=>{
    if(r.children && r.children.length){
      r.children = loopTree(r.children)
    }
    return{
      ...r,
      value:r.id,
...
export function formatterVal(row,DIC,key) {
  const types = useDictionaryStore().getDict(DIC);
